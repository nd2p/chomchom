...
const StarRating = ({ rating, size = 14, showValue = false }) => {
  const stars = [];
  for (let i = 1; i <= 5; i++) {
    stars.push(
      <Text
...
const OverviewTab = ({ story, chaptersCount = 0 }) => (
  <View style={styles.tabContent}>
    ...
    <View style={styles.statsContainer}>
      ...
      <View style={styles.statItem}>
        <StarRating rating={story.ratings} showValue />
        <Text style={styles.statLabel}>{story.ratingCount.toLocaleString()} đánh giá</Text>
      </View>
      <View style={styles.statDivider} />
      <View style={styles.statItem}>
...
const ChaptersTab = ({ chapters }) => (
  <View style={styles.tabContent}>
    ...
    <FlatList
      ...
      renderItem={({ item }) => <ChapterItem chapter={item} onPress={() => item.id && router.push(`/story/chapter/${item.id}`)} />}
      scrollEnabled={false}
    />
  </View>
);
...
const ReviewItem = ({ review }) => (
  <View style={styles.reviewItem}>
    <View style={styles.reviewHeader}>
      <Image source={{ uri: review.user?.avatar || review.avatar || 'https://i.pravatar.cc/100' }} style={styles.avatar} />
...
const ReviewsTab = ({ reviews, comic }) => (
  <View style={styles.tabContent}>
    <View style={styles.reviewSummary}>
      <Text style={styles.ratingBig}>{comic.ratings}</Text>
      <StarRating rating={comic.ratings} size={18} />
...
export default function StoryDetail() {
  ...
  const normalizedComic = {
    ...
    status: comic?.status === 'completed' ? 'Hoàn thành' : 'Đang viết',
    views: comic?.views || 0,
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'overview':
        return <OverviewTab story={normalizedComic} chaptersCount={chapters.length} />;
      case 'chapters':
        return <ChaptersTab chapters={chapters} />;
      case 'reviews':
        return <ReviewsTab reviews={reviews} comic={normalizedComic} />;
      default:
        return <OverviewTab story={normalizedComic} chaptersCount={chapters.length} />;
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bottomActions}>
        ...
        >
          <Text style={styles.readButtonText}>Đọc tiếp</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
